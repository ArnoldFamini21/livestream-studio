...
- [x] Live `/sfu` signaling WebSocket on the media server: purpose-scoped admitted-participant tokens, bound to SfuManager, coexisting with `/rtmp` via a shared upgrade router — verified end-to-end over real sockets (join → publish → layer selection → downlink downgrade → disconnect cleanup)
- [x] SFU media transport (werift, pure TS — no native build): terminates browser Opus audio plus VP8 simulcast publishes, always forwards cloned audio RTP, selectively forwards each subscriber's chosen video layer with live switching/pause, and tears down producers cleanly — audio/video subscribe leg verified over real loopback DTLS/SRTP (media-server/src/sfuTransport.ts)
- [x] SFU browser connection building blocks: purpose-scoped admitted-participant authorization, `/sfu` socket lifecycle, separate publish/subscribe browser peer connections, ICE buffering, and producer-to-track mapping
- [x] Guarded SFU media cutover for 5+ on-stage participants: keep mesh audio/video until the local publish is negotiated and every advertised remote audio/video track is live, preserve per-track mesh fallback for existing stage/recording/broadcast consumers, follow microphone/camera/screen-share changes, and restore mesh automatically on loss or failure
- [x] Simulcast video: browser publishes bounded quality layers and the server selects/forwards the best layer per viewer
- [x] Complete the large-stage media-plane migration by forwarding Opus audio through the SFU and pausing duplicate mesh audio fan-out after guarded cutover
