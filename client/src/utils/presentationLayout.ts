...
export function getPresentationGeometry(layout: LayoutMode, count: number, corner: PresentationCorner = 'BR', size: PresentationCameraSize = 'medium') {
  const plan = getMediaShareLayoutPlan(layout, count);
  const n = plan.visibleParticipantCount;
  const inset = 12, gap = 12;
  const factor = { small: 0.75, medium: 1, large: 1.3 }[normalizePresentationCameraSize(size)];
  ...
  if (plan.placement === 'side-rail' || plan.placement === 'side-by-side') {
    const preferredWidth = (plan.placement === 'side-by-side' ? 288 : 200) * factor;
    const width = Math.min(preferredWidth, ((H - inset * 2 - (n - 1) * gap) / n) * 16 / 9);
    ...
    const width = Math.min(176 * factor, (W - inset * 2 - (n - 1) * gap) / n);
    const height = width * 9 / 16;
    ...
    const width = Math.min((n > 2 ? 172 : 216) * factor, ((H - 40 - (rows - 1) * gap) / rows) * 16 / 9);
    const height = width * 9 / 16;
    for (let i = 0; i < n; i++) {
...
export function getPresentationLayout(layout: LayoutMode, count: number, corner: PresentationCorner, size: PresentationCameraSize = 'medium') {
